    ...
    try {
      ...
      if (email && password) {
        ...
        try {
          ...
          // Если ошибка входа "Invalid login credentials", пробуем зарегистрировать
          if (signInError && signInError.message.includes("Invalid login credentials")) {
            ...
            if (signUpError) {
              // Если ошибка регистрации содержит "User already registered", пробуем войти еще раз
              if (
                signUpError.message.includes("User already registered") ||
                signUpError.message.includes("User already exists") ||
                signUpError.message.includes("User already in use")
              ) {
                // Пробуем войти еще раз с явным указанием пароля
                ...
                if (userProfileError || !userProfile) {
                  throw new Error("Failed to fetch user profile after retry login")
                }
        ...
        // === КАСТОМНЫЙ ЛОГИН/РЕГИСТРАЦИЯ ПО USERNAME ===
