...
export async function GET(
  ...
  try {
    ...
    const formattedGames = (games as any[]).map((game) => {
      ...
      if (game.player_o_user?.username) {
        playerOName = game.player_o_user.username
      } else if (game.players?.O?.username) {
        // Если есть данные в поле players
        playerOName = game.players.O.username
      } else if (game.player_o && game.player_o.startsWith('ai_')) {
        // Если это ИИ, но нет имени, используем "Unknown Player"
        playerOName = "Unknown Player"
      }
